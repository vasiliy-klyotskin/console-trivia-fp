...
def mapToCategoryChoiceViewModel(categories: List[Category]): CategoryChoiceViewModel = {
  val message = "Great! It's time to choose the categories you'd like to play. You can select from a variety of categories. Please enter the category numbers you'd like to include, separated by spaces, or simply type 'all' to play with questions from all categories. Let's get started!"
